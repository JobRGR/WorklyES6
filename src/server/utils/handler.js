...
let Handler = function (name, Module) {
  ...
  return {
    ...
    addItem: (req, res, next) => Module.addItem(req.body, (err, item) => res.send({[this.name]: item})),
    updateItem: (req, res, next) => Module.updateItem(req.params.id, req.body, (err, item) => res.send({[this.name]: item})),
