...
let Handler = function (name, Model, searchItem = true, autocomplete = true) {
  ...
  let handler = {
    ...
    addItems: (req, res, next) => Model.addArray(req.body[this.names], (err, items) => nextItems(err, items, req, next)),
    addItem: (req, res, next) => Model.addItem(req.body[this.name] || req.body.name || req.body, (err, item) => nextItem(err, item, req, next)),
